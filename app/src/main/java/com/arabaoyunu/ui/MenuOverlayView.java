...
public final class MenuOverlayView extends View {
    ...
    private void drawMainMenu(Canvas canvas) {
        ...
        drawButton(canvas, rightCars, "GARAJ / MOD", true, false);
        drawButton(canvas, rightModes, "MOD HUB", true, false);
        drawButton(canvas, rightCareer, "KARIYER", true, false);
        drawButton(canvas, rightMaps, "HARITALAR", false, false);
        drawButton(canvas, rightQuests, TaskAchievementHudSystem.pendingRewardBadge(saveManager), true, false);
        drawButton(canvas, rightSettings, "AYARLAR", false, false);

        paint.setTextAlign(Paint.Align.LEFT);
        paint.setColor(Color.argb(205, 255, 255, 255));
        paint.setTextSize(Math.max(11f, getHeight() * 0.018f));
        canvas.drawText("A63.9: Oyun içi görev takibi, tamamlanma bildirimi ve ödül rozeti aktif.", getWidth() * 0.055f, getHeight() * 0.89f, paint);
    }
    ...
    private void drawMainVehicleGoalCard(Canvas canvas) {
        float min = Math.min(getWidth(), getHeight());
        float x = getWidth() * 0.055f;
        float y = getHeight() * 0.280f;
        float w = getWidth() * 0.365f;
    ...
    private void drawMainModePreview(Canvas canvas) {
        float min = Math.min(getWidth(), getHeight());
        float x = getWidth() * 0.445f;
        float y = getHeight() * 0.280f;
        float w = getWidth() * 0.205f;
        float h = Math.max(140f, getHeight() * 0.225f);
        RectF r = new RectF(x, y, x + w, y + h);
    ...
    private boolean isPreviewArea(float x, float y) {
        ...
        if (screen == GameScreenState.SCREEN_MAIN_MENU || screen == GameScreenState.SCREEN_CAREER_START) {
